...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-beccazeltsman/manhattan.csv")
```
...
What does this mean?
We are 95% confident that the mean price of one- bedroom apartments in Manhattan is between $2302 and $2974
...
We are 95% confident that the median price of one- bedroom apartments in Manhattan is between $2162 and $2875.
...
**Interpretation**:
We are 95% confident that the true proportion of mice that died while not on the treatment is between 69% and 79.7%.
...
```{r practice_2_ci}
ci_mice_alive <- get_ci(boot_prop_alive, level = 0.95)
ci_mice_alive
```
...
visualize(boot_prop_alive) + shade_ci(ci_mice_alive)
```


Give an interpretation of your result.
I am 95% confident that the actual proportion of mice that lived while on the treatment is between 33.7% and 46%
